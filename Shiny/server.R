shinyServer(server <- function(input,output){
  ...
  re_av <- reactive({
    url_av<-paste0(starturl,re_id())
    
    tmp <- jsonlite::fromJSON(url_av)
    
    first <- tmp$firstValue$timestamp %>% convert_sos_date(.) %>% as.Date()
    
    last <- tmp$lastValue$timestamp %>% convert_sos_date(.) %>% as.Date()
  ...
  re_data <- reactive({
    ...
    url1<-paste0(starturl,re_id(),"/getData?timespan=")
    ...
    SOS_data_json <- jsonlite::fromJSON(request)
  ...
  shiny_plot <- reactive({
    Data <- re_data()
    plots <- arrange_SOS_plots(SOS_get_lineplot(Data), SOS_get_boxplot(Data))
    return(plots)
